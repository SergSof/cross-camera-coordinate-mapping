...
По данным видно, что `top` и `bottom` ведут себя как более стабильные камеры, а `door2` имеет заметно плавающую геометрию в рамках одной сессии. Из-за этого задача по входу только `x, y, source` частично неоднозначна: одной и той же координате на `top/bottom` могут соответствовать немного разные координаты на `door2`.
